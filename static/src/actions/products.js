...
export function filterInputSearch(filterCategory: string) {
  return (dispatch: Function) => {
    return axios
      .get(`${ROOT_URL}?limit=15&sort=${filterCategory}`)
...
function formatDateAndPrice(selectAllprices) {
  ...
  selectAllprices.map(element => {
    const formatedPrice =
      '$' + element.price.toFixed(2).replace(/(\d)(?=(\d\d\d)+(?!\d))/g, '$1,');
    const totalDate = [];
...
function dateDiff(unFormatedDate) {
  ...
  if (diff.day > 6) {
    ...
    const getMonthInInteger =
      new Date(Date.parse(substractMonth + ' 1, 2012')).getMonth() + 1;
    const entireDate =
      '0' + getMonthInInteger + ' ' + dateProduct.toString().substr(8, 7);
